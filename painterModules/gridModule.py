...
class colorPallet(pixelArt):
    def setColor(self, colorList): #The colorList argument passed to the function must be equal to the number of pixels in the grid
        ...
        for pixels in self.getGrid():
            for p in pixels:
                p.show(self.screen, colorList[colorCount],0)
                colorCount += 1
...
class menu(grid):
    ...
    def setCMDText(self, textList): #The textList argument passed must be equal to the number of spots in the grid
        ...
        #Set the text for each of those objects
        c = 0
        for i,spots in enumerate(self.getGrid()):
            for j,s in enumerate(spots):
                #s.showText(self.screen, textList[c])
                s.showText(self.screen, textList[(j*5)+i])
                c += 1
